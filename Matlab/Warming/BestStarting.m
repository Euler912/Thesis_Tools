 ...
 function H = barrier_hessian_symmetric(x, A_list, b_list)
    ...
    for i = 1:length(A_list)
        ...
        function x_bar = theorem3_transform(x_star, y_star, alpha, beta, epsilonn, tol)
            ...
            J = finhid(0.5 * (x_star.^2) < y_star - tol);
        ...
        function finding_x_start = iterative_argmax_fg(A_list, b_list, x_mid, f, g, x_g)
            ...
            grad_g = BestStarting.compute_gradient(g, x_g);
            
            for k = 1:max_iter
                % Compute gradient of f at x_old
                grad_f = BestStarting.compute_gradient(f, x_old);
...
function [candidates] = BestStart(f, g, A_list, b_list, n)
        
        % FIND_ARGMAX Finds the argmax and maximum value of F(x) = f(x) - g(x)
        % Inputs:
        %   f       - Objective function to maximize (part of F)
        %   g       - Subtracted function (part of F)
        %   A_list  - Cell array of constraint matrices {A1, A2, ...}
        %   b_list  - Cell array of constraint vectors {b1, b2, ...}
        %   n       - Number of variables
        % Outputs:
        %   x_opt   - Argmax of F(x)
        %   F_opt   - Maximum value of F(x) at x_opt
        ...
        H_f   =BestStarting.hessian_fd(f, x_hat);
        H_phi = BestStarting.barrier_hessian_symmetric(x_ac, A_list, b_list);
        grad_f = BestStarting.compute_gradient(f, x_hat);
        H_g   =BestStarting.hessian_fd(g, x_hat);
    
        % (d) Compute x_mid via the hidden convexity method
        [x_mid, ~] = BestStarting.hidden_convex(H_f, x_hat, grad_f, H_phi, x_ac);
        
        % (e) Additional candidate: maximize (x_mid - x_hat)'*x
        cvx_begin quiet
            variable x(n)
        ...
        candidate_initials = {x_new, x_mid};
        candidates = candidate_initials;
